...
- Your b-threads are **additive**: they will be registered alongside the six existing strategy b-threads, not in place of them. Design your strategy to compose correctly with the existing ones — do not replicate what they already handle.
...
- Track game state exclusively by **observing the event stream** (deal events, card play events, `next_turn`, etc.) — do not use or read from `external_bridge_state` or any external state object. Tracking state via events is the correct BP idiom, as demonstrated by `most_popular_color_selection_strategy`.
...
**Step 3 — Deadlock analysis:** For each b-thread you implement, reason explicitly about deadlock safety:

- For every `block=` clause: explain why the blocked event will always eventually become unblocked (i.e., the blocking condition will eventually cease to hold).
- For every `waitFor=` clause: confirm that there is always at least one other b-thread that will eventually request the awaited event, so the b-thread cannot wait forever.
- If your strategy introduces a new blocking pattern not present in the existing b-threads, pay special attention to interactions with `enforce_card_placement_rules` and `player_behavior`, which control which card events are selectable at any given moment.

---

## PART 6 — Constraints & Notes

- Your b-threads are **additive** — they run alongside the existing six strategy b-threads. Do not attempt to disable, override, or replicate existing behavior. Design for coexistence.
- **Do not use `external_bridge_state`** or any external state object to track game state. The correct BP idiom is to reconstruct state by observing the event stream, as `most_popular_color_selection_strategy` does for hand tracking and color counting.
- `block=` is a powerful BP feature and is permitted in your strategy. Use it when you want to actively prevent certain cards from being played in specific game situations — `strategy_block_super_taki_during_regular_taki` and `prefer_stop_over_regular_cards_strategy` are good examples. However, use it carefully: blocking an event that no other b-thread can unblock, or blocking a lifecycle event, will cause a deadlock.
...
- If your strategy uses multiple b-threads, each must independently follow the game lifecycle. Each b-thread maintains its own local variables — there is no shared mutable state between b-threads.
