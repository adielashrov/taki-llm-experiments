...
class TakiStrategyAgentV1(TakiAgent):
    ...
    def _resulting_active_color(card: str, state: Dict[str, str]) -> str:
        if card == "change_color":
            ...
            remaining_hand = TakiStrategyAgentV1._remove_one(current_hand, card)
            return TakiStrategyAgentV1._best_color(remaining_hand)
        if card == "super_taki":
    ...
    def _best_color(hand: List[str]) -> str:
        ...
        for color in _COLORS:
            weighted = TakiStrategyAgentV1._color_strength(hand, color)
